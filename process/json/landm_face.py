...
Process a video with MediaPipe FaceLandmarker:
  ...
  4. Save a copy of the video with face mesh overlay
...
Usage:
    ...
    python landm_face.py --video path/to/video.mp4 --mesh_video mesh_output.mp4
"""
...
DEFAULT_MESH_VIDEO = None  # will be derived from input if not set
NUM_LANDMARKS = 478

# ---------------------------------------------------------------------------
# Face mesh connection sets (from mediapipe face_mesh.py)
# ---------------------------------------------------------------------------
...
FACEMESH_TESSELATION = frozenset([
    ...
    (390, 339), (339, 249), (249, 390), (448, 261), (261, 255), (255, 448),
    (339, 448), (448, 255), (255, 339)])

# Drawing style definitions: (connections, color_BGR, thickness)
MESH_DRAW_SPECS = [
    # Tesselation: light gray, thin
    (FACEMESH_TESSELATION, (192, 192, 192), 1),
    # Contours: white, slightly thicker
    (FACEMESH_CONTOURS,    (255, 255, 255), 1),
    # Irises: green
    (FACEMESH_IRISES,      (0, 255, 0),     1),
    # Nose: yellow-ish
    (FACEMESH_NOSE,        (0, 200, 200),   1),
]


def draw_face_mesh(frame, landmarks, w, h):
    """Draw the full face mesh overlay on *frame* (modifies in-place)."""
    # Pre-compute pixel positions for all landmarks
    pts = []
    for lm in landmarks:
        px = int(lm.x * w)
        py = int(lm.y * h)
    ...
    for connections, color, thickness in MESH_DRAW_SPECS:
...
def process_video(video_path, model_path, csv_path, bs_path,
    ...
    # --- Set up video writer for mesh overlay ---
    if mesh_video_path is None:
        base, ext = os.path.splitext(video_path)
        mesh_video_path = f"{base}_mesh{ext}"
    fourcc = cv2.VideoWriter_fourcc(*"mp4v")
    mesh_writer = cv2.VideoWriter(mesh_video_path, fourcc, fps, (width, height))
    if not mesh_writer.isOpened():
        print(f"WARNING: Cannot open video writer for {mesh_video_path}")
        mesh_writer = None
        ...
        while True:
            ...
            if result.face_landmarks and len(result.face_landmarks) > 0:
                ...
                if mesh_writer is not None:
                    overlay = frame.copy()
                    draw_face_mesh(overlay, lm_list, width, height)
                    mesh_writer.write(overlay)
    ...
    print(f"Mesh overlay video: {mesh_video_path}")
